...
250823:
串通evaluation前的所有链路。
突然想清楚了一件事情：没有必要非要用负向延迟的数据啊。知道变慢了那直接用原来的不就可以了吗？
todo:
1. rewriting阶段执行延迟时，设置超时时间。--已完成
2. 现在的模型选择必须改革。test析出test和val。
...
26号终于得出第一版本的全链路数据结果了。但是gnn的效果没有bigsubs好。怀疑是批量创建MV批量执行的原因。
后续：测试其他rewriter and 把执行模式修改。支持一个一个创建，一个一个重写。最终取min值

250827:
今天主要是排查gnn为什么没有bigsubs好的原因。书接上文，怀疑一下几个原因：gnn ilp约束问题，批量创建mv批量执行查询进而导致hive用多mv重写的问题（多mv共同写问题），收益估算不准确问题。
首先排查了多mv共同写问题，发现从多写改成单写后，加速比都会下降。但是bigsub的加速比还是要好。由于正好搭建起了单写的workflow，所以**顺带着有考察了一下重新单进程执行重写query的延迟**，发现这回加速比大概能和bigsubs对上了。说明，之前（晚上熬夜写的脚本是直接从stage_summary取得多进程重写后延迟）由于多进程低估了加速比。发现这是一个很重要的提升。
解决方案：今日改成了单进程，并把延迟降低到2min.因为**统计发现bigsubs, gnn真正推荐的mv的创建时间都在1 min以内**。更何况这还是在多进程下的运行时间。单进程的运行时间只会比这个快。
等待单进程结果跑完之后，再重新比较一波结果。
多写问题也确实是一个重要的发现！

250828:
购物（ikea）：买床上用品。前前后后去了big outlet, costco都没有满意的。
wifi事务考察
找家瑭开另外一个gpu机器的权限。
mv：单进程重新stage_summary后的结果：gnn和bigsubs在hive上卧龙凤雏。准备之后试一下其他重写器。其实还有更阴的一招：直接gnn喂数据的时候，把pg重写失败的给过滤掉。
cmpt 354 docker容器封装
...
250830-31:
忙活搬家
31: 把现有的单进程、一个一个创建mv的优化流程集成到了框架中。

250901：
evaluation后续尾巴处理，存excel。$40买了一个超值的冰箱。等R5等了得有七八趟。充实的一天。

250902:
画曲线，得出结果。做会议笔记。

250903：
调研了一下GPU的那两篇论文。lotus可以用，galois基本没有用。讲的完全不是一回事情。

250904:
买了一个超值烤箱$25。老爷爷住的地方让我梦回新加坡。
基本上搞定了enumerator：
   uniview做的修改：喂的query plan是自己的项目生成的。memoize node type不支持，所以做的改动是：获取延迟阶段，把memoize这个node给disable了。
   yang1997做的修改：去掉所有聚合函数，并且把project的列扩展到所有列（这是最重要的一点）。枚举器记录的重写对可能少（这个问题可能也在其他枚举器存在）。不过理论上这个只是影响GNN模型的训练数据和精度。不影响其他。
准备先把这两个enumerator接入现有的这两个推荐模型。先大规模跑实验。
目前正在跑yang1997的stage_summary。完全用枚举器的来。但是估计结果会很少。后续可以考虑是否要用粗子集的方案来配对。autoview则需要首先把register_workload中的query plan给改好。
...
250926:
讨论结果：
原始查询，重写后查询延迟全部设300s。重写后指的是计算time saving
生成训练数据的重写后查询可以依照现有逻辑
MV创建不限制超时时间
5遍改3遍。直接算期值。不再去除最大最小值。
...
明日：
剩余部分全升级完毕（尤其是train test split, rewriter syntax和evaluation改革）。每个workload，都要挑一个跑完全流程。
补丁程序：
...
251019:
费了牛劲才把数据给导进去。验证了跨数据库的数据总条目是否相等。确认相等。除了一个小bug：stats数据库hive有两张表是空。不过确认完全没有影响：首先，本身用的就是空数据库去做的重写，第二，hive和pg走的是同一条路线：表名全部转小写。不过它们也能识别大写的。没有任何问题。
...
251206:
白天：描述性统计基本完成。特别是算子类型的统计。以及思考如何重新做bigsubs的实验让它的效果更差。
写论文：rewriting部分。
...
日程：
16：要基本完结728课程项目。1/2
17：完成LlaVA的FT；对review做全面系统性的梳理。
18-19：调研已有文章。
...
昨天已经核对过了plan。
今天第一要务是完结project。
结果花了好长时间调通qwen。看起来必须得裁剪图像的分辨率。才能把数据塞进去。

260404：
清理小票
清理桌面
搞美签监控程序
llava可以微调，qwen不再微调。直接拿测试集去训练。但是竟然效果还是不好？总之不管怎么办，今天必须截止弄完结果。
...
260409:
完成：
数据：pg dump实例，DSB和TPCDS -5。
查询：tpcds sf5

260411:
复习DL。基本把optimization搞完了。明天收尾一下求偏导。

260418:
在DIS5上跑了DSB workload的前100条query，发现和TPCDS workload类似，在tpcds, dsb两个实例竟然耗时都差不多，而且竟然在tpcds实例运行的耗时还少几秒。
如果为了说明切换数据实例造成的影响，一种方法：专门挑出在tpcds运行快但是放到dsb就慢的query，人工合成一份新的workload。

260422:
日程：
23号1. 把hybrid workload加cache，适当地点增加新workload_name，mlflow field加入新字段。先别忙，有一个重要的问题就是怎么设计workload，这个明天还得仔细想一想。然后也就可以开始跑混合workload的实验了。

260424：
本周末务必把第一部分所有实验都做完。
【正常现象】：
bigsubs枚举器，gnn有512，1024，2048三个区间rewriting阶段跑不通，因为0个重写。
yang1997枚举器，gnn有
...
260507：
确认snowflake不支持带join mv的创建（已经经过多方验证过了）
...
small scale准备数据完毕。明日起正式开始大规模跑训练和重写。

260509：
把bigquery前半部分打通了

260510:
这两天就是密集跑实验，然后要把bigquery的workflow给搞定。
...
260524:
开始重新跑在TPCDS实例上的数据。截止25日中午跑完了hive rewriter的rewriting数据。

260525:
发现小机器实验bigsubs推荐的bug。重新跑实验。并考察cross 是否能够齐平或者超过正常的情况。
截止当日中午，正在跑第二件事情的cross数据。
截止下午三点：
small机器正在跑cross数据

260526:
一地鸡毛。不过总算是封装实验了。明天把所有制表、制图工作完成。
看起来small这一部分还是需要补一下rewritability的数据。不过优先级低。

260528:
基本上完成了今天要干的事情：写一天的论文。明天：把自己写的内容合并到正文中。然后全文通读一遍。

260625:
明日日程：配置6和7-6的环境。看创建mv的效果怎么样。以及考察硬盘的效果。
